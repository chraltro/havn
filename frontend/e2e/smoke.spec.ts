...
/**
 * Navigate to a tab by name. Works for both section tabs (in the header)
 * and sub-tabs (in the sub-tab bar). Clicks the section first if needed,
 * then the sub-tab.
 */
const SECTION_MAP: Record<string, { section: string; subTab?: string }> = {
  // Section-level (no sub-tab needed)
  ...
  Sentinel: { section: "Observe", subTab: "Sentinel" },
  Diff: { section: "Observe", subTab: "Diff" },
  History: { section: "Observe", subTab: "History" },
  // Configure section
  Masking: { section: "Configure", subTab: "Masking" },
  Wiki: { section: "Configure", subTab: "Wiki" },
  Docs: { section: "Configure", subTab: "Docs" },
  Settings: { section: "Configure", subTab: "Settings" },
};
...
test.describe("dp UI smoke tests", () => {
  ...
  test("No uncaught JS errors across all tabs", async ({ page }) => {
    ...
    const allTabs = [
      "Overview", "Editor", "Query", "Tables", "Data Sources",
