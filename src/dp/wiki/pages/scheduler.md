...
dp includes a built-in scheduler for running streams on cron schedules. It also provides a file watcher that automatically rebuilds transforms when SQL files change.
...
dp uses standard 5-field cron expressions:
...
When you run `dp serve`, the scheduler is not started automatically. Start it separately in another terminal:
...
4. For `transform/` changes: runs `dp transform`
...
dp also includes a Huey-based scheduler (SqliteHuey) for more robust task queuing. The SQLite-backed Huey instance stores task state in `.dp_scheduler.db` at the project root.
...
```bash
# Using systemd (Linux)
# Create /etc/systemd/system/dp-scheduler.service

# Using nohup
nohup dp schedule > /var/log/dp-scheduler.log 2>&1 &
```
...
- Check `dp history` to verify scheduled runs are completing
- Use `dp freshness` to detect if scheduled pipelines have stopped running
