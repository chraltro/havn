...
CDC enables incremental data extraction from external sources. Instead of re-fetching all data on every sync, dp tracks what has changed and only fetches new or modified records. This reduces load on source systems and speeds up pipelines.

## CDC Modes

dp supports three CDC modes:
...
```bash
dp cdc status --connector prod_users
```
...
```bash
dp cdc reset --connector prod_users
```

Reset a specific table:

```bash
dp cdc reset --connector prod_users --table users
```
...
5. **Monitor CDC state** -- Use `dp cdc status` regularly to verify syncs are completing and watermarks are advancing.
