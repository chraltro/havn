...
dp provides a comprehensive data quality framework with three complementary systems: inline assertions in SQL models, automatic profiling, and freshness monitoring. For standalone quality rules, see [Contracts](contracts).

## Inline Assertions

Add `-- assert:` comments to SQL model files. Assertions are evaluated after each model builds during `dp transform`:
...
dp automatically computes column-level statistics for every model after it builds. Profiles include:
...
dp profile gold.earthquake_summary
```
...
The `dp check` command runs all quality checks in one pass:
...
Use `dp check` in your CI/CD pipeline:
...
dp supports alerting for pipeline events:
...
alerts:
  ...
  webhook_url: "https://alerts.internal/dp"
  on_success: false
  on_failure: true
```
