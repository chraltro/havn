...
This guide walks you through installing dp, creating your first project, running a pipeline, and exploring data in the web UI.
...
Scaffold a new project with `dp init`:
...
```bash
dp query "SELECT * FROM gold.earthquake_summary LIMIT 10"
```

Output options:

```bash
dp query "SELECT * FROM gold.top_earthquakes" --csv
dp query "SELECT * FROM gold.top_earthquakes" --json
dp query "SELECT COUNT(*) FROM landing.earthquakes" --limit 5
```
