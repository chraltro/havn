...
dp provides two versioning systems: **snapshots** for lightweight project state comparison, and **versions** for full data time travel with Parquet-backed table recovery.
...
```bash
dp snapshot --name before-refactor
```
...
```bash
dp diff --snapshot before-refactor
```
...
In addition to version-based diff, dp supports model-level diff that compares the current SQL output against the materialized table:
...
dp also provides database-level backup and restore:
...
dp restore warehouse.duckdb.backup_20250115_060000
```
