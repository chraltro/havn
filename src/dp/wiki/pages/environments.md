...
dp supports multiple environments so you can maintain separate databases for development, staging, and production. Environments are defined in `project.yml` and selected at runtime via the `--env` flag or the web UI.
...
dp transform --env dev

# Query the production database
dp query "SELECT COUNT(*) FROM gold.customers" --env prod

# Run a stream against staging
dp stream daily-etl --env staging
...
dp resolves variables from a single `.env` file. If you need different secrets per environment, manage this outside dp (e.g., using separate `.env.dev` and `.env.prod` files and symlinking, or using a secrets manager).
...
1. dp loads `project.yml` as normal
