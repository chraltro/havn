...
Contract failures with `error` severity cause `dp check` and `dp contracts` to exit with code 1. Use this for critical data quality rules that must be enforced.
...
```bash
dp contracts orders_not_empty
dp contracts gold.orders
```

### View Contract History

```bash
dp contracts --history
```
...
## Contracts in dp check

The `dp check` command includes contract evaluation:
...
All three must pass for `dp check` to exit with code 0.
...
| Evaluated | During `dp transform` | During `dp contracts` or `dp check` |
...
5. **Run in CI/CD** -- Add `dp contracts` to your CI/CD pipeline to catch quality regressions before deployment.
