...
logger = logging.getLogger("dp.server.rewind")
...
def get_rewind_runs(request: Request, limit: int = 100) -> list[dict]:
    ...
    from dp.engine.snapshots import get_runs
...
def get_rewind_snapshots(request: Request, limit: int = 5000) -> list[dict]:
    ...
    """
    _require_permission(request, "read")
    from dp.engine.snapshots import get_all_snapshots
...
def get_run_snapshots(request: Request, run_id: str) -> list[dict]:
    ...
    _require_permission(request, "read")
    from dp.engine.snapshots import get_snapshots_for_run
...
def get_snapshot_sample_endpoint(
    ...
    _require_permission(request, "read")
    from dp.engine.snapshots import get_snapshot_sample
...
def restore_endpoint(request: Request, req: RestoreRequest, conn: DbConn) -> dict:
    ...
    from dp.engine.snapshots import restore_snapshot, restore_with_cascade
...
def get_downstream_endpoint(request: Request, model_name: str) -> dict:
    ...
    _require_permission(request, "read")
    from dp.engine.snapshots import get_downstream_models
...
def run_gc_endpoint(request: Request) -> dict:
    ...
    _require_permission(request, "execute")
    from dp.engine.snapshots import RewindConfig, run_gc
