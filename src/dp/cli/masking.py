"""Masking policy CLI commands: dp mask list|add|remove."""
...
def mask(
    ...
    Examples:
        dp mask list
        dp mask add --schema gold --table customers --column email --method redact
        dp mask add --schema gold --table customers --column ssn --method partial --show-first 0 --show-last 4
        dp mask remove --id <policy-id>
    """
    from dp.engine.database import connect, ensure_meta_table
    from dp.engine.masking import (
        create_policy,
        delete_policy,
