...
| Snowflake | `snowflake` | Snowflake data warehouse (Arrow transfer via `fetch_arrow_all()`) |
| BigQuery | `bigquery` | Google BigQuery (Arrow transfer via `to_arrow()`) |
| Redshift | `redshift` | Amazon Redshift (via DuckDB postgres extension, zero extra deps) |
