...
# lost and the project stops building.
...
def is_package_path(path: str) -> bool:
    """True when ``path`` lives inside an installed package checkout.

    The path may be project-relative (the usual case) or absolute, so the test
    is on the parts rather than a prefix string: ``havn_packages_backup/x.sql``
    is not a package path.
    """
    return PACKAGES_DIR in Path(path).parts


def _is_package_model(model: Any, path: str) -> bool:
    """True when this model came from an installed package."""
    if getattr(model, "package", None):
        return True
...
def _block_package_sites(report: ReferenceReport) -> None:
    """Move every site inside an installed package out of the edit plan.

    A package checkout is rebuilt by the next ``havn packages install``, which
    deletes the directory: an edit written there is gone by the next install
    and the project stops building because the package still exports the old
    name. The sites are reported as blockers so the reader learns the rename
    reaches into a package and where to make it instead, but nothing is ever
    written there, with or without ``--force``.
    """
    kept: list[RenameSite] = []
    seen: set[str] = set()
    for site in report.sites:
        if not is_package_path(site.path):
            kept.append(site)
            continue
        ...
        report.blocked.append(
            Blocker(PACKAGE, site.model, site.path, PACKAGE_MESSAGE, site.line)
        )
    report.sites = kept
    ...
    *,
    ...
    for site in site_list:
        ...
        if not site.resolved and site.kind != "yaml":
            continue
        if is_package_path(site.path):
            # Also checked here, not only in the index: a caller may pass a
            # bare site list of its own. ``--force`` does not reach inside a
            # package either, because the next install would delete the edit.
            continue
...
def apply_rename(
    ...
    *,
    ...
    for edit in edits:
        if is_package_path(edit.path):
            # The last line of defence. Writing here is always wrong: the
            # checkout is regenerated by `havn packages install`.
            raise RenameError(f"{edit.path}: {PACKAGE_MESSAGE}")
